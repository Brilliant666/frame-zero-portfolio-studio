...
const endpoint = "/api/preview/site-content";
...
export default function PreviewPortfolioAdmin() {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      setConflict(false); setLoadState("ready"); setMessage(result.content ? "新版内容已读取。保存只影响 /preview。" : "尚未配置新版。请填写资料、新建图集后保存；不会自动复制旧站。");
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      setMessage(reconciled.changedWhileSaving ? "提交的版本已保存；保存期间的新编辑仍保留为未保存修改。" : "新版保存成功。/preview 刷新后读取此版本；旧站内容未改变。");
    } catch (error) { setMessage(error instanceof Error ? error.message : "保存失败；草稿保留。"); }
    finally { savingLock.current = false; setSaving(false); }
  }, [conflict, dirty, loadState, revision]);
  useEffect(() => {
  ...
  return <main className={styles.workspace}>
    <header className={styles.topbar}><div><h1>新版摄影作品集后台</h1><p>独立本地工作区 · {dirty ? "有未保存修改" : saved ? `已保存 · 版本 ${revision}` : "尚未配置"}{saving ? " · 保存中" : ""}</p></div><div className={styles.actions}>
      <a href="/preview" target="_blank" rel="noreferrer">查看已保存主页 ↗</a>
    ...
    <div className={styles.body}>
      ...
      <div className={styles.row}><a href="/admin" target="_blank" rel="noreferrer">原版后台 ↗</a><a href="/" target="_blank" rel="noreferrer">原版十一模板主页 ↗</a><span className={styles.hint}>新旧内容独立保存，素材库共用。</span><button type="button" onClick={exportDraft}>导出当前草稿</button></div>
      ...
      <fieldset disabled={loadState !== "ready"} style={{ border: 0, padding: 0, margin: 0, minWidth: 0 }}>
        {section === "profile" && <>
          ...
          <section className={styles.panel}><h2>一次性复制原站资料</h2><p className={styles.hint}>只复制白名单基础资料、套餐和联系方式，不复制模板作品，不猜测图集分类。复制结果先进入草稿。</p><button type="button" onClick={() => void copyBasics()}>从原站复制基础资料到当前草稿</button></section>
        </>}
        {section === "collections" && <>
          ...
          <section className={styles.panel}><h2>显式导入旧原型图集</h2><p className={styles.hint}>选择之前导出的 JSON。仅导入图集到当前草稿，不自动保存；不会猜测分类或恢复已丢失的内存。</p><label className={styles.field}>导入图集 JSON<input type="file" accept="application/json,.json" onChange={(event) => { void importCollections(event.target.files?.[0]); event.target.value = ""; }} /></label></section>
        </>}
        {section === "contact" && <>
    ...
    {preview && createPortal(<section ref={previewRef} className={styles.preview} role="dialog" aria-modal="true" aria-label="新版未保存草稿效果"><div className={styles.previewBar}><p>草稿快照 · 尚未保存到主页</p><button type="button" onClick={() => setPreview(null)}>关闭草稿效果</button></div><PreviewPortfolioView document={preview} embedded initialCollectionId={previewCollectionId} /></section>, document.body)}
  </main>;
}
