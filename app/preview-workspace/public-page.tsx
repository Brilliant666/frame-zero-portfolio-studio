...
export default function PreviewPublicPage() {
  ...
  useEffect(() => {
    ...
    }).catch(error => { if (!controller.signal.aborted) setStatus(error instanceof Error ? error.message : "读取失败，请稍后重试。"); }).finally(() => { if (!controller.signal.aborted) setLoading(false); });
