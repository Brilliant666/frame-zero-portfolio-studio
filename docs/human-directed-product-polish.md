...
> - PR #25 current request: `HR25-007 — READY_FOR_HUMAN_RECHECK`
> - PR #25 next request ID: `HR25-008`
...
Current request: HR25-007 — READY_FOR_HUMAN_RECHECK
Next request ID: HR25-008
...
| HR25-008 | Migration note: this provisional item was first recorded as HR25-005 before implementation, renumbered to HR25-006 on 2026-08-20 when the human explicitly assigned HR25-005 to top-navigation pagination, renumbered to HR25-007 on 2026-08-21 when the human explicitly assigned HR25-006 to the `neon-hud` main-stage orientation fix, and renumbered to HR25-008 later on 2026-08-21 when the human explicitly assigned HR25-007 to the `film-rail` edge-control request. No implementation or commit was attached to any provisional number. Replace link-generated QR output with uploaded QR imagery only after deciding whether one image belongs to each platform or to the whole site, whether upload is an optional override or a complete replacement, whether it must persist after deployment, and which AssetStorage / AssetResolver contract owns upload, public resolution, replacement, and deletion. Do not store image data URLs in content, bind image sidecars to mutable social-array indexes, reuse the works Photo Library, or implement a temporary upload that cannot survive restart. | QR image ownership, content contract, and persistent asset storage | DECISION_REQUIRED | TBD AFTER STORAGE DECISION | NOT_STARTED — human product and architecture decisions required | PENDING |
