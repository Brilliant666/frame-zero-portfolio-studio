...
- 首页通过临时 legacy SSR bridge 在服务端读取 `site_settings(id = 1)`，首屏 HTML 与
  title、description、Open Graph、Twitter metadata 使用同一兼容读取语义；浏览器不再
  二次替换首屏内容。
...
| SSR | legacy `site_settings(id = 1)` 已用于首屏与 metadata；已发布 revision、域名解析和缓存仍未接入 | 部分完成 |
...
写入新模型之前建立。当前 legacy SSR bridge 只消除已保存内容与首屏/SEO 的错位，不是
repository、revision 或 publish 实现；Phase 0 的正式公开读取仍必须只指向已发布 revision。
...
- **PR-05A：真实内容 SSR** — 用站点解析和已发布 revision 读取替换临时
  `site_settings(id = 1)` bridge，并建立正式公开渲染边界。
