...
- GitHub 目前只强制执行公开仓库安全扫描，尚未执行完整 lint、测试、构建和体积门禁。

## Phase 0 差距

| 能力 | 当前状态 | Phase 0 验收状态 |
| --- | --- | --- |
| 项目治理 | NORTH_STAR 已存在，缺少统一 PR / ADR 工作流 | 待实施 |
| CI | 只有公开仓库安全扫描 | 待实施 |
...
- **PR-00A：治理基线** — PR 模板、ADR 模板、Phase 0 跟踪页。
- **PR-00B：完整 CI** — 在公开安全扫描之外执行 lint、测试、构建和体积预算。
- **PR-00C：行为特征测试** — 锁定当前 GET、PUT、本地写入、远端拒绝和失败回退行为。
