...
- “添加素材”是普通本机素材导入的唯一产品主入口；展开后明确提供“选择照片”和“选择文件夹”。照片选择允许一次加入一张或多张，文件夹选择收集包含嵌套目录的本次快照。选择后先显示待处理批次供用户预览和确认；确认前不调用导入服务，取消或重新选择不会改变素材库。
- 文件夹只是批次来源，不作为素材类型或持久化产品数据；后续文件变化不会自动同步，新增素材需再次选择。普通浏览器不会提供可安全持久化的绝对目录路径，记住并重扫来源仍是 `LOCAL_SOURCE_BINDING_FOLLOWUP`。
- 确认导入后更新本地 Photo Library，但不修改 SiteContent draft、不触发自动排版，也不需要点击“保存全部修改”；导入结束自动刷新 manifest。结果摘要使用“本次新增 / 重复跳过 / 素材库总计”，其中重复按文件内容判断，不暗示不同文件名或不同文件夹一定是新素材。“刷新素材列表”只重新读取 manifest，不会扫描电脑文件夹。
